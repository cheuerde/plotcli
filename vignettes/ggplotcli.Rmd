...
`ggplotcli` is a universal converter that renders any ggplot2 plot in the terminal using Unicode/ASCII characters. It automatically extracts data, aesthetics, and styling from ggplot objects.
...
ggplotcli(p, width = 60, height = 40)
```
...
ggplotcli(p, width = 50, height = 12)
```
...
ggplotcli(p + theme_bw(), width = 60, height = 20, border = "auto", grid = "auto")

# theme_classic has border but no grid
ggplotcli(p + theme_classic() + labs(title = "theme_classic() - Border Only"), 
           width = 60, height = 20, border = "auto", grid = "auto")
```
